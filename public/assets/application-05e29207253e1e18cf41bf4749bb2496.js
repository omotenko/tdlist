...
(function() {
  this.tdlist = angular.module("tdlist", ["ngSanitize"]);
  ...
  this.tdlist.factory("middleware", function() {
    return {
      ...
      error: function(data, status) {
        return $scope.errors = '<p><i>' + status + ':' + data + '</i></p>';
      }
    };
  });
...
(function() {
  this.tdlist.controller('CreateCtrl', function($scope, $rootScope, $http, $location, middleware) {
    var HOME_PATH;
    HOME_PATH = '/';
    $scope.save = function() {
      var params;
      ...
      return $http.post('/messages', params).success(middleware.success($location, HOME_PATH)).error(middleware.error);
    };
    return $scope.update = function() {
      ...
      $http.put('/messages/' + params.id, params).success(middleware.success($location, HOME_PATH)).error(middleware.error);
...
(function() {
  ...
  this.tdlist.controller('IndexCtrl', function($scope, $rootScope, $http, $location, middleware) {
    var source;
    ...
    $scope.destroy = function(id) {
      var redirect;
      redirect = '/ok';
      return $http["delete"]('/messages/' + id).success(middleware.success($location, redirect)).error(middleware.error);
    };
    ...
    return $scope.done = function(message) {
      return bootbox.prompt("How much did you (0..100%) ?", function(res) {
        ...
        return $http.put('/messages/' + message.id, params).success(middleware.success($location, '/')).error(middleware.error);
      });
    };
  });

  toJSON = function(messages) {
    var data;

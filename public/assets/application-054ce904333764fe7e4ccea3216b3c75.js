...
(function() {
  this.tdlist.controller('CreateCtrl', function($scope, $rootScope, $http, $location, middleware, storage) {
    ...
    $scope.save = function() {
      ...
      return $http.post('/messages', params).success(middleware.success($location, HOME_PATH)).error(function(data, status) {
        var pendingItems;
        if (status === 0) {
          ...
          return middleware.success($location, HOME_PATH);
        }
      });
    };
    return $scope.update = function() {
      ...
      $http.put('/messages/' + params.id, params).success(middleware.success($location, HOME_PATH)).error(function(data, status) {
        var find, pendingItems;
        if (status === 0) {
          pendingItems = storage.get('pendingItems');
          find = false;
  ...
  this.tdlist.controller('IndexCtrl', function($scope, $rootScope, $http, $location, middleware, storage) {
    ...
    source.onmessage = function(event) {
      return $scope.$apply(function() {
        ...
        return $scope.data = storage.get('pendingItems');
      });
    };
    ...
    $scope.destroy = function(id) {
      var redirect;
      redirect = '/ok';
      return $http["delete"]('/messages/' + id).success(middleware.success($location, redirect)).error(function(data, status) {
        var find, pendingItems;
        if (status === 0) {
          pendingItems = storage.get('pendingItems');
          find = false;
    ...
    return $scope.done = function(message) {
      return bootbox.prompt("How much did you (0..100%) ?", function(res) {
        ...
        return $http.put('/messages/' + message.id, params).success(middleware.success($location, '/')).error(function(data, status) {
          var find, pendingItems;
          if (status === 0) {
            pendingItems = storage.get('pendingItems');
            find = false;
            return angular.forEach(pendingItems, function(key, value) {
              if (!find) {
                if (key.id === params.id) {
                  ...
                  storage.set('pendingItems', $scope.data);
                  find = true;
                  return middleware.success($location, '/ok');
                }
              }
            });
          }
